...
allowed-tools:
  - ToolSearch
  - mcp__watercooler-cloud__watercooler_health
...
1. **Load all diagnostic tools** in parallel:
   ```
   ToolSearch: select:mcp__watercooler-cloud__watercooler_health
...
2. **Run all three checks** in parallel:
   ```
   mcp__watercooler-cloud__watercooler_health(code_path="<repo root>")
