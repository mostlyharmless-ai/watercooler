...
Then call:
```
mcp__watercooler__watercooler_read_thread(topic="<thread-topic>")
```
...
Then call:
```
mcp__watercooler__watercooler_search(query="<key-topic>", mode="entries", semantic=true, code_path="<repo root>")
```

To broaden the search, try additional queries with different topic angles:
```
mcp__watercooler__watercooler_search(query="<related-concept>", mode="entries", semantic=true, code_path="<repo root>")
```
