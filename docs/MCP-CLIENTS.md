...
**`proxy` is the default** — hosted-first, zero local services, with an
automatic fall back to local `stdio` when no hosted credentials are present
(so an open-core / not-yet-authenticated install still runs fully local).
Set a different mode explicitly when the following fit better.
...
and `"proxy"`. Set `transport = "stdio"` to revert to everything-local — the default is
now `proxy` (hosted-first, falling back to local `stdio` without credentials), so
removing the line no longer yields a local install. Restart the local MCP server to apply.
