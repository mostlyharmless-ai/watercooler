...
class ThreadRecord(NamedTuple):
    """Structured thread record returned by _list_threads."""
    title: str
    status: str
    ball: str
    updated: str
...
def _list_threads(
    ...
    Returns:
        List of ThreadRecord named tuples.
    """
    if not _use_graph_for_reads(threads_dir):
    ...
    try:
        ...
        for gt in graph_threads:
            ...
            result.append(ThreadRecord(
                ...
                status=gt.status,
                ball=gt.ball,
                updated=gt.last_updated,
                path=thread_file,
                is_new=is_new,
                summary=gt.summary or "",
                entry_count=gt.entry_count,
            ))
