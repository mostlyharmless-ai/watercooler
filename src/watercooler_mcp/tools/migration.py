...
from ..memory_queue import VALID_BACKENDS  # noqa: E402  # canonical definition
...
async def _migration_preflight_impl(
    ...
    if is_hosted_mode():
        return json.dumps({
            ...
            "message": "Migration tools are not needed in hosted mode. "
                       "Use watercooler_bulk_index to index threads into T2/T3.",
        }, indent=2)
...
async def _migrate_to_memory_backend_impl(
    ...
    if is_hosted_mode():
        return json.dumps({
            "status": "not_applicable",
            "message": "Migration is not applicable in hosted mode. "
                       "Use watercooler_bulk_index to feed threads into memory backends.",
        }, indent=2)
