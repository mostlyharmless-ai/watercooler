...
class ThreadAuditorDaemon(BaseDaemon):
    ...
    def _tick_hosted(self) -> List[Finding]:
        """Hosted mode tick — reads thread data from GitHub API."""
        ...
        for topic in topics:
            ...
            thread_node = get_thread_meta_for_daemon(topic)
            if thread_node is None:
                continue
            ...
            title = thread_node.get("title", "")
            status = thread_node.get("status", "")
            ball = thread_node.get("ball", "")
            last_ts = thread_node.get("last_updated", "")
            has_status_line = bool(status.strip())
            has_ball_line = bool(ball.strip())

            thread_findings = self._audit_thread(
                topic=topic,
                thread_file=None,  # No filesystem path in hosted mode
                threads_dir=Path("/hosted"),  # Sentinel
                title=title,
                status=status,
                ball=ball,
                last_ts=last_ts,
                entries=entries,
                structured=False,  # No filesystem layout in hosted mode
                has_status_line=has_status_line,
                has_ball_line=has_ball_line,
                thread_node=thread_node,
            )
