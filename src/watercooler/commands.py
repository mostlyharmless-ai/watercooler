...
def _post_compact_has_capture_hook(entries: list) -> bool:
    """Return True if a capture hook entry exists in a PostCompact hook list."""
    for entry in entries:
...
def setup_pulse_hook(
    settings_path: Path | None = None,
    local_settings_path: Path | None = None,
) -> int:
    """Wire watercooler-capture-theme as a PostCompact hook.

    Checks settings.local.json first for an existing hook, then settings.json.
    Writes to settings.json by default to preserve current behavior.
    ...
    # Resolve watercooler-capture-theme.
    # The capture script is always installed alongside the watercooler CLI, so
    # check the sibling bin directory of the running executable first.  This
    # covers the common case where the venv is not activated and the console
    # scripts are not on PATH (e.g. `uv run watercooler setup-pulse-hook` or
    # invocation via an absolute path from the compatibility shim).
    _capture_name = "watercooler-capture-theme"
    _self_dir = Path(sys.argv[0]).resolve().parent
    _sibling = _self_dir / _capture_name
    if _sibling.is_file() and os.access(_sibling, os.X_OK):
        hook_bin = str(_sibling)
    else:
        hook_bin = shutil.which(_capture_name)
    if not hook_bin:
        print(
            f"❌ {_capture_name} not found (checked {_self_dir} and PATH).\n"
    ...
    def _has_capture_hook(path: Path) -> bool:
        """Return True if a capture hook already exists in path.

        Raises json.JSONDecodeError if the file exists but contains invalid JSON
        so the caller can surface a clear error rather than silently ignoring a
        malformed higher-priority settings file.
        """
        if not path.exists():
            return False
        try:
        ...
        if not isinstance(pc, list):
            return False
        return _post_compact_has_capture_hook(pc)
    ...
    try:
        if _has_capture_hook(_local_settings_path):
            print(f"✅ Capture hook already configured in {_local_settings_path}")
            return 0
    except json.JSONDecodeError as e:
        print(
            f"❌ {_local_settings_path} contains invalid JSON: {e}\n"
            f"   Fix or remove it before running setup-pulse-hook.",
            file=sys.stderr,
        )
        return 1
    ...
    if hooks_val is None:
        ...
        print(
            f"❌ {_settings_path}: 'hooks' key is not an object (got {type(hooks_val).__name__})",
            file=sys.stderr,
        )
        return 1

    hooks = settings["hooks"]
    pc_val = hooks.get("PostCompact", [])
    if not isinstance(pc_val, list):
        print(
            f"❌ {_settings_path}: 'PostCompact' key is not an array (got {type(pc_val).__name__})",
            file=sys.stderr,
        )
        return 1
    post_compact: list = pc_val

    # Check if hook already exists in settings.json
    if _post_compact_has_capture_hook(post_compact):
        print(f"✅ Capture hook already configured in {_settings_path}")
        return 0

    # Append new hook entry.
    # Claude Code requires the nested `{"hooks": [{...}]}` envelope for PostCompact —
    # the bare `{"type": "command", ...}` shape is accepted by the loader but no
    # stdin payload is piped to the command, so the capture hook sees empty input
    # and fails the JSON parse. See bug-postcompact-hook-stdin-empty.
    post_compact.append({"hooks": [{"type": "command", "command": hook_bin}]})
    hooks["PostCompact"] = post_compact
    ...
    print(f"✅ PostCompact hook configured: {hook_bin}")
