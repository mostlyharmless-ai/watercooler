...
class HostedCredentials(BaseModel):
    """Hosted MCP endpoint credentials.

    The api_key is a per-user Bearer token (wc_...) created in the
    dashboard Settings -> Security -> Agent API Keys. All agents on
    this machine use it automatically for hosted MCP connections.
    """

    api_key: str = Field(
        default="",
        description="Agent API key for hosted MCP (wc_... from dashboard)",
    )
...
class Credentials(BaseModel):
    ...
    # Hosted MCP endpoint
    hosted: HostedCredentials = Field(default_factory=HostedCredentials)
