...
class Config:
    ...
    def get_mode(self) -> str:
        """Get the product mode (local or hosted).

        Precedence: WATERCOOLER_MODE env > TOML config > default ("local")

        Returns:
            "local" or "hosted"
        """
        env_mode = self.env.get("WATERCOOLER_MODE", "")
        if env_mode:
            mode = env_mode.lower().strip()
            if mode in ("local", "hosted"):
                return mode
            logging.warning(
                f"Invalid WATERCOOLER_MODE={env_mode!r}, "
                f"expected 'local' or 'hosted'. Falling back to config."
            )
        try:
            return self.full().mode
        except Exception as e:
            logging.error(f"Failed to load mode from config: {e}. Defaulting to 'local'.")
            return "local"
    ...
    def is_hosted_mode(self) -> bool:
        ...
        Returns:
            True if mode is "hosted"
        """
        return self.get_mode() == "hosted"

    def get_hosted_api_key(self) -> str:
        """Get the hosted MCP API key from credentials.toml [hosted].api_key.

        Returns:
            The wc_... key, or empty string if not configured.
        """
        try:
            from .credentials import load_credentials
            creds = load_credentials(auto_migrate=False)
            return creds.hosted.api_key
        except Exception as e:
            logging.warning(f"Failed to load hosted API key: {e}")
            return ""
