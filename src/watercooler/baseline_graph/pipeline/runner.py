...
class BaselineGraphRunner:
    ...
    def _export_graph(self, threads: List[Any]) -> tuple[int, int]:
        ...
        # Write per-topic search index shards
        search_by_topic: Dict[str, List[Dict[str, Any]]] = {}
        orphan_entries: List[Dict[str, Any]] = []
        for sie in search_index_entries:
            t = sie.get("thread_topic", "")
            if t:
                search_by_topic.setdefault(t, []).append(sie)
            else:
                orphan_entries.append(sie)
        for t, entries in search_by_topic.items():
            shard_path = storage.get_thread_graph_dir(graph_dir, t) / "search-index.jsonl"
            storage.atomic_write_jsonl(shard_path, entries)
        # Entries without thread_topic go to the global file as fallback
        if orphan_entries:
            self._log(f"  Warning: {len(orphan_entries)} search index entries have no thread_topic")
            storage.atomic_write_jsonl(graph_dir / "search-index.jsonl", orphan_entries)
