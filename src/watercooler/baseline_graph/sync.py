...
def get_memory_backend_config() -> Optional[Dict[str, Any]]:
    ...
    # ValueError means an invalid backend name was configured — treat as disabled
    # so sync_to_memory_backend() degrades gracefully (returns False, never raises).
    try:
        backend = get_memory_backend()
    except ValueError as exc:
        logger.warning("MEMORY: %s — backend disabled", exc)
        return None
