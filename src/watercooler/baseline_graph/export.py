...
def export_thread_to_per_thread(
    thread: ParsedThread,
    output_dir: Path,
) -> Tuple[int, int]:
    """Export a single thread to per-thread graph format.

    Writes to output_dir/threads/<topic>/{meta.json, entries.jsonl, edges.jsonl}.
    ...
    """
    thread_dir = output_dir / "threads" / thread.topic
    ...
    # Write meta.json
    meta = thread_to_node(thread)
    with open(thread_dir / "meta.json", "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    nodes_written += 1

    # Write entries.jsonl
    with open(thread_dir / "entries.jsonl", "w", encoding="utf-8") as f:
        for entry in thread.entries:
            f.write(json.dumps(entry_to_node(entry, thread.topic)) + "\n")
            nodes_written += 1

    # Write edges.jsonl
    with open(thread_dir / "edges.jsonl", "w", encoding="utf-8") as f:
...
def export_all_threads(
    ...
    """Export all threads to per-thread graph format.

    Writes per-thread graph files to output_dir/threads/<topic>/.
