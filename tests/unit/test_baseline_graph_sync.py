...
def test_full_sync_stamps_summary_schema_version(threads_dir: Path, sample_thread: Path):
    """Summaries generated by a full sync are stamped, so missing-mode enrich does not
    treat them as stale and regenerate repeatedly (P2)."""
    from watercooler.baseline_graph.summarizer import (
    ...
    with patch(
        "watercooler.baseline_graph.sync.summarize_thread",
        return_value="A factual discussion summary.",
    ), patch(
        "watercooler.baseline_graph.sync.summarize_entry",
        return_value="entry summary",
    ):
    ...
    meta_file = (
        threads_dir / "graph" / "baseline" / "threads" / "test-topic" / "meta.json"
    )
    meta = json.loads(meta_file.read_text(encoding="utf-8"))
    assert meta.get("summary")
    assert meta.get("summary_schema_version") == SUMMARY_SCHEMA_VERSION
    assert summary_is_stale(meta) is False


def test_enrich_thread_summaries_respects_limit(threads_dir: Path):
    """The stale-summary migration must be bounded by `limit` so it can drain in
    batches rather than regenerating the whole corpus in one call (P2)."""
    ...
    for topic in ("topic-a", "topic-b", "topic-c"):
        ...
        # No "summary" / no version => stale, eligible for thread-summary regeneration.
        storage.atomic_write_json(thread_dir / "meta.json", {
            "id": f"thread:{topic}", "type": "thread", "topic": topic,
            "title": topic, "status": "OPEN", "ball": "", "entry_count": 1,
            "last_updated": "2025-01-01T00:00:00Z",
        })
        storage.atomic_write_jsonl(thread_dir / "entries.jsonl", [{
            "id": f"entry:{topic}-1", "entry_id": f"{topic}-1", "index": 0,
            "agent": "a", "role": "planner", "entry_type": "Note",
            "title": "E", "timestamp": "2025-01-01T00:00:00Z", "body": "b",
        }])

    # dry_run avoids any LLM; only the thread-summary path is exercised.
    result = enrich_graph(
        threads_dir, summaries=False, embeddings=False, thread_summaries=True,
        mode="all", limit=1, dry_run=True,
    )
    assert result.thread_summaries_generated == 1
...
def test_manifest_updated_on_sync(threads_dir: Path, sample_thread: Path):
    """Test manifest is rebuilt from scan after sync."""
    sync_thread_to_graph(threads_dir, "test-topic")

    # Manifest is now a derived cache — load_manifest() scans per-thread meta
    from watercooler.baseline_graph.storage import load_manifest, get_graph_dir
...
""",
    ...
    # Both should be in manifest (scan-based rebuild)
    from watercooler.baseline_graph.storage import load_manifest, get_graph_dir
    graph_dir = get_graph_dir(threads_dir)
    manifest = load_manifest(graph_dir)
...
def test_sync_to_memory_backend_graphiti(threads_dir: Path, sample_thread: Path, monkeypatch):
    ...
    monkeypatch.setenv("WATERCOOLER_MEMORY_BACKEND", "graphiti")
    # Force legacy executor path — real TOML has queue_enabled=true which would
    # bypass the callback entirely and route to the memory queue instead.
    monkeypatch.setattr("watercooler.memory_config.is_memory_queue_enabled", lambda: False)
...
def test_check_graph_health_basic(thread_with_graph: Path, threads_dir: Path):
    """Test check_graph_health reports healthy state."""
    report = check_graph_health(threads_dir)
