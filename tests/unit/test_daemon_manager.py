...
from watercooler_mcp.daemons.errors import DaemonAlreadyRegisteredError
...
class TestDaemonManager:
    ...
    def test_get_all_findings_daemon_not_registered_falls_back_to_disk(
        self, tmp_path, monkeypatch
    ):
        """When daemon isn't in this process, fall back to JSONL on disk."""
        monkeypatch.setattr(
            "watercooler_mcp.daemons.state._DEFAULT_DAEMONS_DIR", tmp_path
        )
        mgr = DaemonManager()
        # No JSONL file exists → returns empty list instead of raising
        findings = mgr.get_all_findings(daemon="ghost", limit=10)
        assert findings == []
