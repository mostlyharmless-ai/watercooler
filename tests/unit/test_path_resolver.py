...
class TestWorktreeDiscovery:
    """Tests for orphan branch worktree discovery in resolve_threads_dir."""

    def test_worktree_exists_returns_worktree_path(self, tmp_path, monkeypatch):
        """Test resolve_threads_dir returns worktree path when it exists."""
        # Set up fake worktree base
        worktree_base = tmp_path / "worktrees"
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", worktree_base)

        # Create worktree with .git marker
        repo_name = "my-repo"
        worktree = worktree_base / repo_name
        worktree.mkdir(parents=True)
        (worktree / ".git").write_text("gitdir: /some/path")

        # Create a fake git repo so discover_git_info returns a root
        code_root = tmp_path / repo_name
        code_root.mkdir()
        git_info = GitInfo(root=code_root, branch="main", commit="abc1234", remote=None)

        with patch("watercooler.path_resolver.discover_git_info", return_value=git_info):
            result = resolve_threads_dir(code_root=code_root)

        assert result == worktree.resolve()

    def test_worktree_no_git_marker_falls_through(self, tmp_path, monkeypatch):
        """Test worktree dir without .git marker is ignored."""
        worktree_base = tmp_path / "worktrees"
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", worktree_base)

        # Create worktree directory WITHOUT .git marker
        repo_name = "my-repo"
        worktree = worktree_base / repo_name
        worktree.mkdir(parents=True)
        ...
        # Should fall through to sibling directory
        expected = (code_root.parent / f"{repo_name}-threads").resolve()
        assert result == expected

    def test_no_worktree_sibling_fallback(self, tmp_path, monkeypatch):
        """Test falls through to sibling when no worktree exists."""
        worktree_base = tmp_path / "worktrees"
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", worktree_base)
        # worktree_base doesn't even exist
    ...
    def test_env_var_wins_over_worktree(self, tmp_path, monkeypatch):
        """Test WATERCOOLER_DIR env var takes precedence over worktree."""
        worktree_base = tmp_path / "worktrees"
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", worktree_base)
    ...
    def test_cli_arg_wins_over_worktree(self, tmp_path, monkeypatch):
        """Test --threads-dir CLI arg takes precedence over worktree."""
        worktree_base = tmp_path / "worktrees"
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", worktree_base)
        ...
        cli_dir = tmp_path / "cli-threads"
        result = resolve_threads_dir(cli_value=str(cli_dir))
        assert result == cli_dir.resolve()

    def test_discover_worktree_valid(self, tmp_path, monkeypatch):
        """Test _discover_worktree with valid worktree."""
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", tmp_path)
        worktree = tmp_path / "my-repo"
        worktree.mkdir()
        (worktree / ".git").write_text("gitdir: /some/path")

        result = _discover_worktree("my-repo")
        assert result == worktree.resolve()

    def test_discover_worktree_missing(self, tmp_path, monkeypatch):
        """Test _discover_worktree with nonexistent repo."""
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", tmp_path)
        result = _discover_worktree("no-such-repo")
        assert result is None

    def test_discover_worktree_no_git_file(self, tmp_path, monkeypatch):
        """Test _discover_worktree with dir but no .git marker."""
        monkeypatch.setattr("watercooler.path_resolver.WORKTREE_BASE", tmp_path)
