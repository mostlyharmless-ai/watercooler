...
class TestPathTraversalPrevention:
    ...
    def test_validate_topic_rejects_path_traversal(self):
        ...
        assert _validate_topic("../etc/passwd") is not None
        assert _validate_topic("..\\windows\\system32") is not None
        assert _validate_topic("foo/../bar") is not None
        assert _validate_topic("foo/bar") is not None
        assert _validate_topic("foo\\bar") is not None
    ...
    def test_validate_topic_rejects_hidden_files(self):
        ...
        assert _validate_topic(".hidden") is not None
        assert _validate_topic(".env") is not None

    def test_validate_topic_rejects_empty(self):
        """Topic validation rejects empty strings."""
        from watercooler_mcp.hosted_ops import _validate_topic

        assert _validate_topic("") is not None
    ...
    def test_validate_topic_allows_valid_topics(self):
        ...
        assert _validate_topic("feature-auth-refactor") is None
        assert _validate_topic("v2-api-design") is None
        assert _validate_topic("sprint-42-planning") is None
...
class TestManifestLocking:
    ...
    def test_manifest_uses_advisory_lock(self, tmp_path):
        """Manifest loads from scan when file absent."""
        from watercooler.baseline_graph import storage

        graph_dir = tmp_path / "graph" / "baseline"
        graph_dir.mkdir(parents=True)

        # Create a per-thread meta.json for scan-based rebuild
        topic_dir = graph_dir / "threads" / "test-topic"
        topic_dir.mkdir(parents=True)
        (topic_dir / "meta.json").write_text(
            '{"id": "thread:test-topic", "topic": "test-topic", '
            '"last_updated": "2025-01-01T00:00:00Z"}',
            encoding="utf-8",
        )

        # update_manifest is now a no-op (deprecated)
        storage.update_manifest(graph_dir, "test-topic", "entry-1")

        # load_manifest should rebuild from scan
