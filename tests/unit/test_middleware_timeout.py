...
def _reset_orig_run():
    """Reset instrumentation state between tests to prevent recursive wrapping
    and ensure each test gets a fresh setup_instrumentation() call."""
    saved_orig_run = _mw._orig_run
    saved_installed = _mw._instrumentation_installed
    _mw._orig_run = None
    _mw._instrumentation_installed = False
    yield
    _mw._orig_run = saved_orig_run
    _mw._instrumentation_installed = saved_installed
...
def _instrument(tool_cls):
    ...
    with mock.patch.dict("sys.modules", {"fastmcp.tools.function_tool": fake_module}):
        setup_instrumentation()
