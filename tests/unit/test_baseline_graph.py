...
class TestExportAndLoad:
    ...
    def test_export_thread_to_per_thread(self, tmp_path):
        """Test exporting a single thread to per-thread format."""
        entry = ParsedEntry(
            entry_id="test:1",
            index=1,
            agent="Claude",
        ...
        nodes, edges = export_thread_to_per_thread(thread, output_dir)

        assert nodes == 2  # 1 thread meta + 1 entry
        assert edges == 1  # 1 contains edge
        thread_dir = output_dir / "threads" / "test"
        assert (thread_dir / "meta.json").exists()
        assert (thread_dir / "entries.jsonl").exists()
        assert (thread_dir / "edges.jsonl").exists()
...
class TestExportAllThreads:
    ...
    def test_export_all_threads(self, tmp_path):
        ...
        thread_dir = output_dir / "threads" / "test-topic"
        assert (thread_dir / "meta.json").exists()
        assert (thread_dir / "entries.jsonl").exists()
        assert (output_dir / "manifest.json").exists()

    def test_export_overwrites_existing_per_thread(self, tmp_path):
        """Test that export overwrites existing per-thread data."""
        from watercooler.baseline_graph.writer import init_thread_in_graph, upsert_entry_node, EntryData
        ...
        # Per-thread data should exist
        thread_dir = output_dir / "threads" / "test"
        assert (thread_dir / "meta.json").exists()
        meta_content = (thread_dir / "meta.json").read_text()
        assert "thread:test" in meta_content
