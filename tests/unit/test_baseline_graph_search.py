...
class TestSearchGraph:
    ...
    def test_search_query_operator_or(self, populated_graph):
        """Test OR query operator matches when any token is present."""
        query = SearchQuery(
            query="jwt redirect",
            query_operator="OR",
            include_threads=False,
        ...
        results = search_graph(populated_graph, query)
        entry_ids = {result.node_id for result in results.results}
        assert "01JXYZ123456" in entry_ids
        assert "01JXYZ123458" in entry_ids
